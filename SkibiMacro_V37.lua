-- ============================================================================== --
-- // SKIBI DEFENSE - FLUENT MACRO EDITION V37 [SMART POSITION RECOVERY]
-- // V37 Fix: AstroJugg pos บันทึกผิด (X,Z = นอกแผนที่ Y=78 ลอยฟ้า)
-- //          → ใช้ "Smart Recovery" หา ground position ใกล้เคียงด้วย Raycast spiral
-- //          → เพิ่ม Manual Override: user คลิกบนพื้นเองตอน Play ถ้า macro ติด
-- //          → Force string conversion ทุก argument (กัน Instance leak)
-- // V36: ForceGroundY (ดี แต่ raycast นอกแผนที่ไม่เจอพื้น)
-- ============================================================================== --
...
local Window = Fluent:CreateWindow({
    Title = "Skibi Macro V37",
    SubTitle = "Smart Position Recovery",
    TabWidth = 160,
...
local function StartRecordingProcess()
    ...
    Fluent:Notify({ Title = "Recording Started", Content = "เริ่มอัดมาโคร V37!", Duration = 3 })
    StartObserving()
end
        ...
        for i = 1, actionCount do
            ...
            elseif step.type == "Place" then
                ...
                repeat
                    ...
                    -- ⭐ V37: Force string ทุก argument กัน Instance leak
                    pcall(function()
                        PlaceRemote:FireServer(tostring(step.unit), currentTryPos, false)
                    end)
...
dbg("SkibiMacro V37 loaded successfully!")
Fluent:Notify({
    Title = "Loaded V37",
    Content = "Smart Recovery + Manual Click ถ้า macro ติด",
    Duration = 5
})
