-- ============================================================================== --
-- // SKIBI DEFENSE - FLUENT MACRO EDITION V38 [UNIT NAME MAP FIX]
-- // V38 Fix: AstroJugg = upgrade form ของ UpgradedTitanTVMan (UTTVM)
-- //          → ใช้ UNIT_NAME_MAP แปลงชื่อ display → ชื่อ place จริง
-- //          → ตอนนี้ macro ที่มี AstroJugg จะวางเป็น UTTVM แล้วอัพเกรดต่อปกติ
-- // V37: Smart Recovery + Manual Override + Spiral Search
-- // V36: ForceGroundY (raycast)
...
-- ============================================================================== --
-- // ⭐ V38: UNIT NAME MAP
-- // บาง unit เป็น "upgrade form" ที่วางตรงๆ ไม่ได้
-- // ต้อง map ชื่อ display → ชื่อ unit จริงที่ส่งใน placeTower
-- ============================================================================== --
local UNIT_NAME_MAP = {
    -- AstroJugg (และ skin XFAres) คือ upgrade form ของ UpgradedTitanTVMan
    AstroJugg = "UpgradedTitanTVMan",
    -- เพิ่มเติมในอนาคตได้ ถ้าเจอ unit อื่นที่เป็น upgrade form เช่น:
    -- ["SomeOtherSkinName"] = "RealBaseUnit",
}

local function ResolvePlaceName(displayName)
    if not displayName then return displayName end
    local mapped = UNIT_NAME_MAP[displayName]
    if mapped then
        dbg(string.format("Map: %s → %s", displayName, mapped))
        return mapped
    end
    return displayName
end
...
local Window = Fluent:CreateWindow({
    Title = "Skibi Macro V38",
    SubTitle = "Unit Name Map Fix",
    TabWidth = 160,
...
-- ⭐ V37: Manual override — user คลิกบนพื้นเพื่อบอกตำแหน่งจริง
local manualOverridePos = nil            -- เก็บตำแหน่งล่าสุดที่ user คลิกตอน play
local awaitingManualClick = false        -- flag บอกว่ากำลังรอ user คลิก
...
-- ⭐ V37: Spiral search — หาพื้นใกล้เคียงถ้าจุดเดิมไม่เจอ
-- Return: array of {x, y, z} positions ที่เจอพื้น, เรียงจากใกล้ไปไกล
local function FindNearbyGroundPositions(centerX, centerZ, ignoreList, maxRadius, samples)
    maxRadius = maxRadius or 50
    samples = samples or 8
    local results = {}
    
    -- ลองจุดเดิมก่อน
    local centerY = RaycastGroundY(centerX, centerZ, ignoreList)
    if centerY then
        table.insert(results, { x = centerX, y = centerY, z = centerZ, dist = 0 })
    end
    ...
    for radius = 5, maxRadius, 5 do
        for i = 0, samples - 1 do
            ...
            local y = RaycastGroundY(x, z, ignoreList)
            if y then
                table.insert(results, { x = x, y = y, z = z, dist = radius })
            end
        end
        if #results >= 5 then break end  -- เจอพอแล้ว
    end
...
local function GetUnitByPosition(targetName, targetPosCf)
    ...
    -- ⭐ V38: ค้นหาทั้งชื่อ display และชื่อ mapped (สำหรับ upgrade-form units)
    local searchName = CleanStr(targetName)
    local mappedName = ResolvePlaceName(targetName)
    local searchNameMapped = (mappedName ~= targetName) and CleanStr(mappedName) or nil
    ...
    for _, unit in ipairs(targetFolder:GetChildren()) do
        ...
        local matched = string.find(uName, searchName, 1, true)
            or string.find(sId, searchName, 1, true)
        
        -- ⭐ ถ้ายังไม่ match ลอง mapped name ด้วย
        if not matched and searchNameMapped then
            matched = string.find(uName, searchNameMapped, 1, true)
                or string.find(sId, searchNameMapped, 1, true)
        end
...
-- ⭐ V37: ปุ่ม Skip step ปัจจุบัน
local skipCurrentStep = false
Tabs.Main:AddButton({
    Title = "⏭️ Skip Current Action (ถ้าติด)",
    Callback = function()
        if isReplaying then
            skipCurrentStep = true
            Fluent:Notify({
                Title = "Skipped",
                Content = "จะข้าม step ปัจจุบันใน 1-2 วิ",
                Duration = 3
            })
        else
            Fluent:Notify({ Title = "Not Playing", Content = "ยังไม่ได้เริ่ม Play", Duration = 2 })
        end
    end
})
...
Tabs.Settings:AddButton({
    ...
    Callback = function()
        ...
        if hit then
            ...
            local placeName = ResolvePlaceName("AstroJugg")
            print(string.format("[V38 TEST] Place '%s' (mapped from AstroJugg) at: %s",
                placeName, tostring(testCf.Position)))
            local ok, err = pcall(function()
                PlaceRemote:FireServer(placeName, testCf, false)
            end)
            print("[V38 TEST] Result:", ok, err)
            Fluent:Notify({
                Title = "Test fired",
                Content = "ส่ง: " .. placeName .. " - ดู console (F9)",
                Duration = 5
            })
        else
...
UserInputService.InputBegan:Connect(function(input, gpe)
    if input.UserInputType == Enum.UserInputType.MouseButton1 and not gpe then
        ...
        -- ⭐ V37: ถ้ากำลัง play แล้ว user คลิก → เก็บไว้เป็น manual override
        if isReplaying and awaitingManualClick then
            pcall(function()
                manualOverridePos = mouse.Hit.Position
                dbg("[V37] Manual override captured:", manualOverridePos)
                Fluent:Notify({
                    Title = "Manual Override",
                    Content = string.format("รับตำแหน่ง (%.1f, %.1f, %.1f)",
                        manualOverridePos.X, manualOverridePos.Y, manualOverridePos.Z),
                    Duration = 2
                })
            end)
        end
...
local function StartRecordingProcess()
    ...
    Fluent:Notify({ Title = "Recording Started", Content = "เริ่มอัดมาโคร V38!", Duration = 3 })
    StartObserving()
end
...
local function PlayMacroData()
    ...
    task.spawn(function()
        ...
        -- ⭐ V37: helper เช็คว่าควร skip step ปัจจุบันไหม
        local function shouldSkipStep()
            if skipCurrentStep then
                skipCurrentStep = false  -- reset flag
                return true
            end
            return false
        end
        ...
        for i = 1, actionCount do
            ...
            elseif step.type == "Place" then
                ...
                -- ⭐ V37 Smart Recovery: สร้าง list ตำแหน่งให้ลองหลายๆแบบ
                ...
                if targetPosCf then
                    ...
                    -- 1. ลอง pos เดิมจาก JSON ก่อน
                    table.insert(fallbackPositions, {
                        cf = targetPosCf,
                        label = "JSON original"
                    })
                    
                    -- 2. ลอง raycast ลงพื้นจาก X,Z เดิม
                    local groundY = RaycastGroundY(targetPosCf.X, targetPosCf.Z, ignoreList)
                    if groundY then
                        table.insert(fallbackPositions, {
                            cf = CFrame.new(targetPosCf.X, groundY, targetPosCf.Z),
                            label = string.format("Raycast Y=%.1f", groundY)
                        })
                    end
                    
                    -- 3. ⭐ Spiral search หาพื้นใกล้เคียง (สำหรับ flying unit ที่ pos ผิด)
                    if forceGround then
                        local nearby = FindNearbyGroundPositions(
                            targetPosCf.X, targetPosCf.Z, ignoreList, 60, 12
                        )
                        for _, p in ipairs(nearby) do
                            -- ข้ามจุดที่ใกล้กับ groundY มาก (ซ้ำกับข้อ 2)
                            if p.dist > 0 then
                                table.insert(fallbackPositions, {
                                    cf = CFrame.new(p.x, p.y, p.z),
                                    label = string.format("Spiral d=%d Y=%.1f", p.dist, p.y)
                                })
                            end
                        end
                    end
                    
                    dbg(string.format("[Place %s] Generated %d fallback positions",
                        step.unit, #fallbackPositions))
                end
                
                local fallbackIdx = 1
                local switchEvery = 10  -- สลับทุกๆ 10 attempts
                local manualPromptShown = false
                
                repeat
                    -- ⭐ ใช้ Manual Override ถ้ามี (priority สูงสุด)
                    local currentTryPos
                    if manualOverridePos then
                        currentTryPos = CFrame.new(
                            manualOverridePos.X,
                            manualOverridePos.Y,
                            manualOverridePos.Z
                        )
                        UpdateStatus("Playing", i, step.type, step.unit, "🎯 Manual Click Pos")
                    else
                        local entry = fallbackPositions[fallbackIdx]
                        currentTryPos = entry and entry.cf or targetPosCf
                        
                        -- สลับ fallback
                        if attempts > 0 and attempts % switchEvery == 0 and #fallbackPositions > 1 then
                            fallbackIdx = (fallbackIdx % #fallbackPositions) + 1
                            entry = fallbackPositions[fallbackIdx]
                            currentTryPos = entry and entry.cf or targetPosCf
                            dbg(string.format("[Place %s] Try fallback #%d: %s",
                                step.unit, fallbackIdx, entry and entry.label or "?"))
                        end
                        
                        -- ⭐ ถ้าลองทุก fallback แล้วยังไม่ได้ → ขอ user คลิกเอง
                        if attempts >= switchEvery * #fallbackPositions and not manualPromptShown then
                            manualPromptShown = true
                            awaitingManualClick = true
                            UpdateStatus("Playing", i, step.type, step.unit,
                                "🖱️ Click ground to place " .. step.unit .. "!")
                            Fluent:Notify({
                                Title = "Manual Help Needed",
                                Content = "คลิกบนพื้นที่อยากวาง " .. step.unit,
                                Duration = 8
                            })
                        end
                    end
                    
                    -- ⭐ V38: Map ชื่อ display → ชื่อ place จริง (เช่น AstroJugg → UpgradedTitanTVMan)
                    local placeName = ResolvePlaceName(tostring(step.unit))
                    
                    -- Force string ทุก argument กัน Instance leak
                    pcall(function()
                        PlaceRemote:FireServer(placeName, currentTryPos, false)
                    end)
                    ...
                    if targetFolder then
                        -- ⭐ V38: ค้นด้วยชื่อที่ map แล้ว เพราะ unit ที่เกิดมาจะมี sID = mapped name
                        local searchNameClean = CleanStr(placeName)
                        local bestDist = CONFIG.UNIT_SEARCH_MAX_DIST
                        local bestUnit = nil
                        
                        for _, unit in ipairs(targetFolder:GetChildren()) do
                            if not IsUnitOwned(unit) then
                    ...
                    if foundUnit then
                        isPlaced = true
                        SetPlayInstance(step.targetID, foundUnit)
                        manualOverridePos = nil  -- ใช้แล้วล้าง
                        awaitingManualClick = false
                    end
                    
                    if not isPlaced and not manualOverridePos then
                        local entry = fallbackPositions[fallbackIdx]
                        UpdateStatus("Playing", i, step.type, step.unit,
                            string.format("Retry %d/%d [%s]",
                                attempts, CONFIG.MAX_PLACE_ATTEMPTS,
                                entry and entry.label or "?"))
                    end
                    attempts = attempts + 1
                until isPlaced or attempts >= CONFIG.MAX_PLACE_ATTEMPTS or shouldStop() or shouldSkipStep()
                
                awaitingManualClick = false
                manualOverridePos = nil
                ...
                if not isPlaced then
                    ...
                    Fluent:Notify({
                        Title = "Skipped",
                        Content = "ข้าม " .. step.unit .. " ไป (วางไม่ได้)",
                        Duration = 3
                    })
                    task.wait(0.5)
                end
            ...
            elseif step.type == "Upgrade" then
                ...
                repeat
                    ...
                    if unitToUpgrade then
                        ...
                        -- ⭐ V37: Force string conversion ทุก argument
                        pcall(function() UpgradeRemote:FireServer(tostring(currentIdStr)) end)
                        if currentIdNum then
                            -- ลอง number ด้วย (บาง remote รับทั้ง 2 type)
            ...
            elseif step.type == "Sell" then
                ...
                repeat
                    ...
                    if unitToSell then
                        ...
                        -- ⭐ V37: Force string conversion ทุก argument
                        pcall(function() SellRemote:FireServer(tostring(currentIdStr)) end)
                        if currentIdNum then
...
dbg("SkibiMacro V38 loaded successfully!")
Fluent:Notify({
    Title = "Loaded V38",
    Content = "Unit Name Map: AstroJugg → UpgradedTitanTVMan",
    Duration = 5
})
