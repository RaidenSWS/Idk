-- ============================================================================== --
-- // SKIBI DEFENSE - FLUENT MACRO EDITION V34 [PERFECT]
-- // Improvements: Fixed Money Debounce Bug, Reverse Lookup, Pre-cached Raycast,
-- //               Safe JSON Parse, Configurable Constants, Memory Cleanup,
-- //               Removed _G usage, Better Timeouts, Smart Adaptive Delay
-- ============================================================================== --
...
local Window = Fluent:CreateWindow({
    Title = "Skibi Macro V34",
    SubTitle = "Perfect Edition",
    TabWidth = 160,
...
local function RecordAction(actionType, targetId, posCf, unitName, exactTime, specificLevel)
    ...
    -- ⭐ เก็บ groundY pre-cached สำหรับ Place เพื่อไม่ต้อง raycast ตอน playback
    if actionType == "Place" and posCf then
        stepData.groundY = posCf.Y
    end
...
local function StartObserving()
    ...
    local addConn = towerDataFolder.ChildAdded:Connect(function(tDataObj)
        ...
        -- ⭐ Pre-cache ground position ตอน record (เร็วกว่า raycast ตอน play)
...
local function StartRecordingProcess()
    ...
    Fluent:Notify({ Title = "Recording Started", Content = "เริ่มอัดมาโคร V34!", Duration = 3 })
    StartObserving()
end
...
local function PlayMacroData()
    ...
    task.spawn(function()
        ...
        for i = 1, actionCount do
            ...
            -- ⭐ สร้าง CFrame จาก data — ใช้ groundY ที่ pre-cached แล้ว
            local targetPosCf = nil
            local p = ParseCFrameStr(step.pos)
            if p then
                pcall(function()
                    if step.type == "Place" then
                        -- ใช้ groundY ที่เก็บไว้ตอน record (ไม่ต้อง raycast ใหม่)
                        local y = step.groundY or p[2] or 0
                        targetPosCf = CFrame.new(p[1] or 0, y, p[3] or 0)
                    else
                        if #p >= 12 then
...
dbg("SkibiMacro V34 loaded successfully!")
Fluent:Notify({ Title = "Loaded", Content = "Skibi Macro V34 พร้อมใช้งาน!", Duration = 3 })
