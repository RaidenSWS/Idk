-- ============================================================================== --
-- // SKIBI DEFENSE - FLUENT MACRO EDITION V35 [FLYING UNIT FIX]
-- // V35 Fix: AstroJugg (flying unit) Place — เลิก raycast ทับ Y, ใช้ตำแหน่งจริง
-- // V34 Improvements: Money Debounce, Reverse Lookup, Safe JSON, Configurable
-- // V33 Base: Adaptive Delay, Caching, Memory Cleanup
-- ============================================================================== --
...
local Window = Fluent:CreateWindow({
    Title = "Skibi Macro V35",
    SubTitle = "Flying Unit Fix",
    TabWidth = 160,
...
local function StartObserving()
    ...
    local addConn = towerDataFolder.ChildAdded:Connect(function(tDataObj)
        ...
        -- ⭐ V35 FIX: ใช้ตำแหน่งจริงของ unit (รองรับ flying unit เช่น AstroJugg)
        -- หลักการใหม่: เชื่อ unit position ก่อน, raycast เป็นแค่ fallback
        local posCf = CFrame.new(0, 0, 0)
        local actualUnitCf = nil
        
        if unitModel then
            actualUnitCf = unitModel.PrimaryPart and unitModel.PrimaryPart.CFrame or GetModelCFrame(unitModel)
        end
        
        if actualUnitCf then
            -- ⭐ ใช้ตำแหน่งจริงของ unit ทั้ง X, Y, Z
            -- ป้องกัน flying units (AstroJugg) ถูก raycast บังคับลงพื้น
            posCf = CFrame.new(actualUnitCf.X, actualUnitCf.Y, actualUnitCf.Z)
        elseif lastGroundPos then
            -- fallback: ถ้าหา unit ไม่เจอ ใช้ mouse.Hit (ground unit เท่านั้น)
...
local function StartRecordingProcess()
    ...
    Fluent:Notify({ Title = "Recording Started", Content = "เริ่มอัดมาโคร V35!", Duration = 3 })
    StartObserving()
end
            ...
            elseif step.type == "Place" then
                ...
                repeat
                            ...
                            if not IsUnitOwned(unit) then  -- O(1) check
                                ...
                                if string.find(uNameClean, searchNameClean, 1, true)
                                    ...
                                    if cf and targetPosCf then
                                        local dx = cf.Position.X - targetPosCf.Position.X
                                        local dz = cf.Position.Z - targetPosCf.Position.Z
                    ...
                    if not foundUnit then
                        foundUnit = GetUnitByPosition(step.unit, targetPosCf)
                    end
                    ...
                    if not isPlaced then
                        UpdateStatus("Playing", i, step.type, step.unit,
                            string.format("Place retry (%d/%d)", attempts, CONFIG.MAX_PLACE_ATTEMPTS))
                    end
                    attempts = attempts + 1
...
dbg("SkibiMacro V35 loaded successfully!")
Fluent:Notify({ Title = "Loaded", Content = "Skibi Macro V35 พร้อมใช้งาน!", Duration = 3 })
